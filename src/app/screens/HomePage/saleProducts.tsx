...
export function SaleProducts(props: SaleProductsProps) {
  /** INITIALIZATIONS */
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { saleProducts } = useSelector(saleProductsRetriever);

  // ✅ ALWAYS ARRAY (map crash stop)
  const saleProductsList: Product[] = useMemo(() => {
    return Array.isArray(saleProducts) ? saleProducts : [];
  }, [saleProducts]);
  ...
  useEffect(() => {
    ...
    productService
      ...
      .then((data: any) => {
        // ✅ API response ba'zida [] yoki { data: [] } bo'lishi mumkin
        const list: Product[] = Array.isArray(data) ? data : (data?.data ?? []);
        dispatch(setSaleProducts(list)); // ✅ unstable function emas, direct dispatch
      })
      .catch((err) => console.log(err));
  }, [productRebuild, dispatch]); // ✅ setSaleProducts emas

  const formatTimeRemaining = (endTime: any): string => {
    const now = new Date();
    const endDate = new Date(endTime);
    const diff = endDate.getTime() - now.getTime();

    if (!endTime || Number.isNaN(endDate.getTime()) || diff <= 0) {
      return "00:00:00";
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeRemainingArray(
        saleProductsList.map((product: Product) =>
  ...
  // ✅ e.target.id o'rniga direct productId yuboramiz (barqaror)
  const targetLikeProduct = async (productId: string) => {
    try {
      assert.ok(verifiedMemberData, Definer.auth_err1);

      const memberService = new MemberApiService();
      const like_result: any = await memberService.memberLikeTarget({
        like_ref_id: productId,
        group_type: "product",
      });
      ...
      setProductRebuild(new Date()); // ✅ refresh fetch
    } catch (error: any) {
  ...
  if (isMobile()) {
    return (
      <div className="p_products_sale" style={{ width: "430px" }}>
        <Container sx={{ overflow: "hidden" }}>
          <Stack className={"p_products_main"}>
            <Box className={"p_products_text"}>
              <span className={"title"} style={{ marginTop: "20px", fontSize: "36px" }}>
                Products on Sale
              </span>
            </Box>
            ...
            <Stack className="swiper">
              ...
              >
                {saleProductsList.map((product: Product, index: number) => {
                  const image_path = `${serverApi}/${product?.product_images?.[0] ?? ""}`;
                  const discountedPrice = Math.floor(product?.discountedPrice ?? 0);

                  return (
                    <SwiperSlide className={"product_info_frame"} key={product?._id ?? index}>
                      <Stack
                            ...
                            <Box className={"dish_sale"}>
                              <span className={"dish_sale-txt"}>
                                {product?.discount?.type === "amount" ? (
                                  <Box className="discount_fon">-{product?.discount?.value}$</Box>
                                ) : (
                                  <Box className="discount_fon">-{product?.discount?.value}%</Box>
                                )}
                              </span>
                      ...
                      <Stack className={"product_name"} style={{ marginLeft: "90px" }}>
                        {product?.product_name}
                      </Stack>

                      <Stack className={"rating_box"} style={{ marginLeft: "20px" }}>
                        <Rating
                      ...
                      <Stack className={"price"} style={{ marginLeft: "170px" }}>
                        ...
                        >
                          ...
                          {product?.discountedPrice ? (
                            <>
                              <span style={{ color: "orange", position: "relative" }}>
                  ...
                  <SwiperSlide className={"product_info_frame"} key={product?._id ?? index}>
                    <Stack className={"product-box"} onClick={() => chosenProductHandler(product._id)}>
                      <Box className={"img"} sx={{ backgroundImage: `url(${image_path})` }}>
                        ...
                        >
                          <Badge badgeContent={product?.product_likes ?? 0} color="secondary">
                            <Checkbox
                              icon={<FavoriteBorder style={{ color: "white" }} />}
                              checkedIcon={<Favorite style={{ color: "red" }} />}
                              checked={!!product?.me_liked?.[0]?.my_favorite}
                              onClick={(e) => {
                                e.stopPropagation();
                                targetLikeProduct(product._id);
                              }}
                            />
                          </Badge>
                        </Button>
                        ...
                        <Button className={"like_view_btn"} style={{ right: "36px" }}>
                          <Badge badgeContent={product?.product_reviews ?? 0} color="secondary">
